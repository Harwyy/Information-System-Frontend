...
class SpecialOperationAPI {
  async updateEmployeeCount(id) {
    const response = await fetch(
      `${API_BASE_URL}/special-operation/update-count-employee?id=${id}`,
      {
        method: 'PUT',
        headers: {
  ...
  async getMaxOfficialAddress() {
    const response = await fetch(
      `${API_BASE_URL}/special-operation/max-official-address`
    );
  ...
  async getGroupedFullName() {
    const response = await fetch(
      `${API_BASE_URL}/special-operation/group-fullname`
    );
  ...
  async joinOrganizations(requestData) {
    const response = await fetch(
      `${API_BASE_URL}/special-operation/join-organizations`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
